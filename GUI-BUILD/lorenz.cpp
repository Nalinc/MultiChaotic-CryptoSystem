...
double Lorenz::rand_float(double low, double high)
{
    return ( ( double )rand() * ( high - low ) ) / ( double )RAND_MAX + low;
}


void Lorenz::compute(double x, double *y, double *eq) 
{
/*
  eq[0] = - y[1] - y[2];
  eq[1] = y[0] + (a * y[1]);
...
void Lorenz::init()
{
  pFile = fopen ("lorenz_datum.dat","w");

  printf("# SYSTEM: Lorenz Attractor--- \n");
...
fprintf(pFile,"# SYSTEM: Lorenz Attractor--- \n");
fprintf(pFile,"#\tt\t\t\tx[0] \t\t\t x[1] \t\t\t x[2] \t\t\t y[0] \t\t\t y[1] \t\t\t y[2] \n");

 
  for(t=BEGINl; t<=ENDl; t+=DTl)
  {
     x[1] = y[1];
     rk4(Nl, t, x, DTl);
     rk4(Nl, t, y, DTl);
...
int Lorenz::rk4(int n, double x, double *y, double h)
{
